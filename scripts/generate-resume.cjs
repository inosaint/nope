...
// Helper function for work items
function workItem(date, title, description) {
  ...
  if (description) {
    doc.fontSize(10).font('Helvetica').fillColor(gray).text(description, { lineGap: 2 });
  }
  doc.moveDown(0.8);
}
...
doc.moveDown(0.3);
doc.fontSize(10).font('Helvetica').fillColor(gray).text('noopur190@gmail.com  |  noopur.me', { align: 'left' });
